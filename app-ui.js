...
function buildCardInner(card, detailed){
  ...
  return `
    <div class="card-preview">
      <div class="card-head">
        <span class="icon card-head-icon-wrap"><img class="card-head-icon" src="${iconAsset}" alt=""></span>
        <span class="badge">${vis.label}</span>
      </div>
      <div class="card-art has-art" style="background-image:url('${art}')">
        <div class="card-art-badge">C${card.cost}</div>
        ${eye}
      </div>
      <div class="card-mini-name">${card.name}</div>
      <div class="card-mini-effect">${effectText}</div>
      <div class="card-mini-foot"><span>${detailed ? card.rarity : ""}</span><span>${detailed ? t("card_modal_details") : ""}</span></div>
    </div>
  `;
}
