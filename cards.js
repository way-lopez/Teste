...
function getCardArtAsset(cardOrId){
  ...
  return "assets/cards/card_" + mapped + ".webp";
}

function getCardIconAsset(cardOrId){
  const card = typeof cardOrId === "string" ? getCard(cardOrId) : cardOrId;
  if(!card) return "assets/icons/icon_attack.webp";

  const byId = {
    curepoison: "icon_antidote.webp",
    revitalize: "icon_heal.webp",
    nerfar: "icon_nerfar.webp",
    rezet: "icon_rezet.webp",
    exorcism: "icon_exorcism.webp",
    hemodrain: "icon_curse.webp",
    garlicward: "icon_antidote.webp",
    forcedheal: "icon_exorcism.webp",
    imm1: "icon_regen.webp",
    imm3: "icon_regen.webp",
    imm5: "icon_regen.webp",
    reb2: "icon_rebote.webp",
    reb3: "icon_rebote.webp",
    reb5: "icon_rebote.webp",
    rebmaster: "icon_rebote.webp"
  };
  if(byId[card.id]) return "assets/icons/" + byId[card.id];

  if(card.id.indexOf("poison") === 0) return "assets/icons/icon_poison.webp";
  if(card.id.indexOf("vamp") === 0) return "assets/icons/icon_vamp.webp";
  if(card.id.indexOf("buff") === 0) return "assets/icons/icon_buff.webp";
  if(card.id.indexOf("curse") === 0) return "assets/icons/icon_curse.webp";
  if(card.id.indexOf("heal") === 0) return "assets/icons/icon_heal.webp";
  if(card.id.indexOf("block") === 0 || card.id.indexOf("defense") === 0) return "assets/icons/icon_defense.webp";
  if(card.id.indexOf("atk") === 0 || card.id.indexOf("sacrifice") === 0) return "assets/icons/icon_attack.webp";

  const byType = {
    attack: "icon_attack.webp",
    defense: "icon_defense.webp",
    buff: "icon_buff.webp",
    debuff: "icon_curse.webp",
    support: "icon_heal.webp",
    special: "icon_energy.webp"
  };
  return "assets/icons/" + (byType[card.type] || "icon_attack.webp");
}
